...
<body class="bg-slate-50 text-slate-900">
    <main class="max-w-3xl mx-auto px-6 py-12">
        <div class="mb-12">
            <a href="{{ route('events.show', $event->id) }}" class="text-indigo-600 font-bold flex items-center gap-2 mb-6 hover:gap-3 transition">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <form action="{{ route('checkout.store', $event->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 mt-2 font-bold uppercase text-tighter">*E-Ticket akan dikirim ke email ini</p>
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">No. WhatsApp</label>
                            <input type="tel" name="customer_phone" placeholder="08xxxxxx"
                                class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm sticky top-32">
                    ...
                    <div class="flex gap-6 items-start mb-8 pt-6 border-t space-y-3">
                        ...
                        <div>
                            ...
                            <p class="text-slate-500">{{ \Carbon\Carbon::parse($event->date)->format('d M Y') }}</p>
